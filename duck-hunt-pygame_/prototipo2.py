...
class DialogueSystem:
    """Sistema de diálogos con voz y efectos visuales"""
    def __init__(self, ambient_sound):
        ...
        self.story_dialogues = [
            # Capítulo 1: El Amanecer del Cazador
            ...
            # Modo Secreto
            {"speaker": "???", "text": "Has encontrado el camino prohibido...", "voice": "thunder"},
            {"speaker": "Voz Oscura", "text": "En esta cueva moran criaturas ancestrales...", "voice": "thunder"},
            {"speaker": "Voz Oscura", "text": "Demuestra tu valor, cazador...", "voice": "thunder"},
        ]
        
        # Diálogos de combate
        self.combat_dialogues = [
            {"speaker": "Hunter", "text": "¡Ahí van!", "trigger": "first_shot"},
            {"speaker": "Hunter", "text": "¡Buen tiro!", "trigger": "combo"},
            {"speaker": "Hunter", "text": "¡No puedes escapar!", "trigger": "duck_escape"},
            {"speaker": "Hunter", "text": "¡Perfecto!", "trigger": "perfect_round"},
            {"speaker": "Hunter", "text": "¡Necesito recargar!", "trigger": "low_ammo"},
            {"speaker": "Hunter", "text": "¡Poder activado!", "trigger": "powerup"},
            {"speaker": "Hunter", "text": "¡Modo dios!", "trigger": "god_mode"},
            {"speaker": "Hunter", "text": "¡A por ellos!", "trigger": "boss_spawn"},
        ]
    
    def add_story_dialogue(self, chapter, index):
        """Añade diálogo de historia basado en capítulo"""
        if chapter == 1 and index < 2:
            self.dialogues.append(self.story_dialogues[index])
        elif chapter == 2 and index < 2:
            self.dialogues.append(self.story_dialogues[index + 2])
        elif chapter == 3 and index < 2:
            self.dialogues.append(self.story_dialogues[index + 4])
        elif chapter == 4 and index < 2:
            self.dialogues.append(self.story_dialogues[index + 6])
    
    def add_combat_dialogue(self, trigger):
        """Añade diálogo de combate basado en evento"""
        for dialogue in self.combat_dialogues:
            if dialogue["trigger"] == trigger:
                self.dialogues.append(dialogue)
                break
    
    def update(self):
        """Actualiza el sistema de diálogos"""
        ...
        if not self.current_dialogue and self.dialogues:
            ...
            self.dialogue_timer = 180  # 3 segundos
            # Reproducir sonido de voz si existe
    ...
    def draw(self, surface):
        ...
        if self.current_dialogue:
            # Fondo del diálogo
            dialogue_height = 100
            dialogue_y = SCREEN_HEIGHT - dialogue_height - 20
            
            # Fondo semi-transparente
            overlay = pygame.Surface((SCREEN_WIDTH - 40, dialogue_height), pygame.SRCALPHA)
            overlay.fill((0, 0, 0, 200))
            surface.blit(overlay, (20, dialogue_y))
            
            # Borde
            pygame.draw.rect(surface, GOLD, (20, dialogue_y, SCREEN_WIDTH - 40, dialogue_height), 2, border_radius=10)
            
            # Texto del hablante
            speaker_text = font_medium.render(f"{self.current_dialogue['speaker']}:", True, GOLD)
            surface.blit(speaker_text, (40, dialogue_y + 15))
            
            # Texto del diálogo
            dialogue_text = font_small.render(self.current_dialogue['text'], True, WHITE)
            surface.blit(dialogue_text, (40, dialogue_y + 55))
            
            # Indicador para continuar
            if self.dialogue_timer < 60:
                pulse = int(128 + 127 * math.sin(pygame.time.get_ticks() * 0.005))
                continue_text = font_tiny.render("Presiona ESPACIO para continuar", True, (pulse, pulse, pulse))
                surface.blit(continue_text, (SCREEN_WIDTH // 2 - continue_text.get_width() // 2, dialogue_y + 80))
...
class SoundSystem:
    ...
    def generate_all_sounds(self):
        ...
        try:
            ...
            for i in range(samples):
                t = i / sample_rate
                freq = 300 + math.sin(t * 20) * 100
                envelope = 1.0 - t / duration
                val = int(3000 * math.sin(2 * math.pi * freq * t) * envelope)
                arr.append(max(-32767, min(32767, int(val * 0.3))))
            self.sounds['dialogue'] = pygame.mixer.Sound(buffer=bytes(b''.join(v.to_bytes(2, 'little', signed=True) for v in arr)))
            ...
            self.sounds['dialogue'].set_volume(self.volume * 0.4)
...
class QTESystem:
    ...
    def draw(self, surface):
        ...
        # Título
        title = font_medium.render("¡REACCIONA RÁPIDO!", True, WHITE)
...
# ============ CLASES DE PARTÍCULAS Y EFECTOS ============
class Particle:
    ...
    def update(self):
        self.x += self.vx
        self.y += self.vy
        if self.type not in ['star', 'ember', 'smoke']:
            self.vy += 0.15
        elif self.type == 'ember':
            self.vy -= 0.02  # Sube
            self.vx += random.uniform(-0.1, 0.1)
        elif self.type == 'smoke':
            self.size += 0.1  # El humo se expande
    ...
    def draw(self, surface):
        ...
        if self.type == 'feather':
            ...
            size = max(2, int(self.size * (self.life / self.max_life)))
            pygame.draw.circle(surface, (*self.color[:3], alpha // 2), (int(self.x), int(self.y)), size)
...
class Duck:
    """Clase para los patos con mejoras gráficas"""
    def __init__(self, difficulty=1.0, is_boss=False, boss_data=None):
        ...
        self.escaping = False
        self.rotation = 0
        self.scale = 1.0 if not is_boss else 1.8
        self.hit_flash = 0
        self.invulnerable = 0
        self.phase = 0  # Para animaciones de boss
    ...
    def update(self, slow_motion=False):
        ...
        if self.alive:
            ...
            if self.escaping:
                ...
                if self.is_boss:
                    # Movimiento especial para bosses
                    self.phase += 0.05
    ...
    def draw(self, surface):
        # Dibujar partículas de estela primero
        ...
        if self.is_boss and self.invulnerable > 0:
            ...
            alpha = int(128 + 127 * math.sin(self.phase * 5))
        ...
        if self.is_boss and self.health > 0 and not self.falling:
            bar_width = int(70 * scale)
            bar_x = x + int(DUCK_WIDTH * scale // 2) - bar_width // 2
            bar_y = y - 15
            
            # Fondo de la barra
            pygame.draw.rect(surface, (50, 50, 50), (bar_x, bar_y, bar_width, 8))
            
            # Barra de vida
            health_ratio = self.health / self.max_health
            health_width = int(bar_width * health_ratio)
            
            # Color basado en la vida restante
            if health_ratio > 0.6:
                health_color = GREEN
            ...
            pygame.draw.rect(surface, health_color, (bar_x, bar_y, health_width, 8))
    ...
    def check_hit(self, mx, my):
        if self.alive and not self.falling and self.invulnerable <= 0:
            ...
            if dist < hit_radius:
                ...
                self.hit_flash = 15
                if self.is_boss:
                    self.invulnerable = 20  # Invulnerabilidad breve después de ser golpeado
...
class Dog:
    ...
    def activate_god_mode(self):
        ...
        if self.god_mode_used_this_round:
            return False  # Ya se usó esta ronda
        self.god_mode = True
    ...
    def draw(self, surface):
        ...
        # Dibujar partículas de humo primero
...
def draw_background(surface, clouds, time_of_day='day', stars=None, frame=0):
    """Dibuja el fondo mejorado según la hora del día"""
...
def draw_dead_tree(surface, x, y, scale=1.0):
    """Dibuja un árbol muerto para el infierno"""
...
def draw_intro_screen(surface, frame, menu_selection, secret_mode_unlocked=False):
    """Dibuja la pantalla de introducción mejorada"""
    # Animated background
    ...
    # Agregar modo secreto si está desbloqueado
...
def draw_chapter_intro(surface, chapter, frame):
    """Dibuja la intro de cada capítulo con animación"""
    surface.fill((0, 0, 0))
...
def draw_story_intro(surface, frame, stage):
    """Dibuja la intro de la historia del perro"""
    ...
    if stage == 0:  # ESCENA 1: Familia feliz del perro
        ...
        # Perro padre Hunter (más detallado)
        ...
        # Cachorros (más detallados y animados)
    ...
    elif stage == 1:  # ESCENA 2: Tiempos difíciles (más emotiva)
    ...
    elif stage == 2:  # ESCENA 3: La decisión heroica
        # Fondo amanecer dramático
        for y in range(SCREEN_HEIGHT):
            ratio = y / SCREEN_HEIGHT
...
SECRET_BOSSES = [
    {"name": "SOMBRA ALADA", "description": "El guardián de la cueva", "color": (60, 60, 80), "health": 3, "points": 1000},
    {"name": "FÉNIX OSCURO", "description": "Renace de las cenizas", "color": (200, 50, 30), "health": 4, "points": 1500},
    {"name": "ESPECTRO DEL VACÍO", "description": "Maestro de las sombras", "color": (100, 50, 150), "health": 5, "points": 2000},
    {"name": "DEMONIO CARMESÍ", "description": "Señor del infierno", "color": (180, 20, 20), "health": 6, "points": 2500},
    {"name": "TITÁN INFERNAL", "description": "El jefe final", "color": (255, 80, 0), "health": 8, "points": 5000},
]
...
def main():
    ...
    # Sistema de diálogos (pasa ambient_sound como parámetro)
    dialogue_system = DialogueSystem(ambient_sound)
    
    # Variables modo secreto
    secret_mode = False
    secret_mode_unlocked = False  # Si el modo secreto está desbloqueado
    ...
    required_score = 0
    showing_unlock_animation = False  # Mostrar animación de desbloqueo
    unlock_animation_frame = 0
    
    # Variable para controlar la presentación de bosses
    showing_boss_intro = False
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if game_state == 'intro':
                    # Solo 3 opciones en menú (modo secreto se desbloquea en juego)
                    ...
                    if event.key == pygame.K_w:
                        ...
                        if menu_selection == 0:  # Story Mode
                            ...
                            secret_mode = False
                            ambient_sound.stop()
                            # Añadir diálogos de historia
                            dialogue_system.add_story_dialogue(1, 0)
                            dialogue_system.add_story_dialogue(1, 1)
                            ...
                            dog.reset_round()
                            ambient_sound.stop()
                            dialogue_system.add_combat_dialogue('first_shot')
                            ...
                            # Añadir diálogos de modo secreto
                            for i in range(9, 12):  # Diálogos 9-11 son del modo secreto
                                if i < len(dialogue_system.story_dialogues):
                                    dialogue_system.dialogues.append(dialogue_system.story_dialogues[i])
                    ...
                    if event.key == pygame.K_SPACE and chapter_frame > 90:
                        ...
                        if secret_intro_stage > 2:
                            showing_boss_intro = True
                            current_boss_index = 0
                            chapter_frame = 0
                            ambient_sound.play('boss')
                
                elif game_state == 'boss_presentation' or showing_boss_intro:
                    if event.key == pygame.K_SPACE and chapter_frame > 80:
                        if current_boss_index < len(SECRET_BOSSES) - 1:
                            ...
                            # Crear el primer boss
                            ducks = [Duck(difficulty=round_num + 1, is_boss=True, boss_data=SECRET_BOSSES[0])]
                            ducks_spawned = 1
                            ducks_hit = 0
                            ammo = 3
                            dog.reset_round()
                    ...
                    if event.key == pygame.K_SPACE and chapter_frame > 50:
                        ...
                        floating_texts = []
                        dog.reset_round()
                        # Añadir diálogo de combate para el primer disparo
                        dialogue_system.add_combat_dialogue('first_shot')
                
                elif game_state == 'playing':
                    # Combo detection - FUNCIONA EN TODOS LOS MODOS
                    key_map = {pygame.K_w: 'W', pygame.K_a: 'A', pygame.K_s: 'S', pygame.K_d: 'D'}
                    if event.key in key_map:
                        combo_keys.append(key_map[event.key])
                        combo_timer = 90
                        
                        if len(combo_keys) > 8:  # Para el combo secreto WSADWSAD (8 teclas)
                        ...
                        if combo_str.endswith('AWSD'):
                            if dog.activate_god_mode():  # Solo si no se ha usado esta ronda
                                ...
                                dialogue_system.add_combat_dialogue('god_mode')
                                for _ in range(20):
                            ...
                            combo_keys = []
                            combo_display = ""
                            floating_texts.append(FloatingText(SCREEN_WIDTH // 2, 200, "¡MUNICIÓN INFINITA!", (255, 180, 50), 'medium'))
                        elif combo_str.endswith('WDS'):
                            active_powerups['slow_motion'] = POWERUP_DURATION_STANDARD
                            combo_keys = []
                            combo_display = ""
                            floating_texts.append(FloatingText(SCREEN_WIDTH // 2, 200, "¡CÁMARA LENTA!", (100, 150, 255), 'medium'))
                            ...
                            dialogue_system.add_combat_dialogue('powerup')
                        
                        # COMBO SECRETO PARA DESBLOQUEAR MODO SECRETO: WSADWSAD
                    ...
                    if event.key == pygame.K_SPACE and ammo > 0 and not showing_unlock_animation:
                        ...
                        for duck in ducks:
                            ...
                            if result == True:  # Kill
                                ...
                                # Diálogo de buen tiro
                                if random.random() < 0.3:
                                    dialogue_system.add_combat_dialogue('good_shot')
                                break
                        ...
                        for powerup in powerups[:]:
                            ...
                            if ptype:
                                if ptype == 'rapid_fire':
                                    ...
                                    floating_texts.append(FloatingText(powerup.x, powerup.y, "¡DISPARO RÁPIDO!", GOLD))
                                    dialogue_system.add_combat_dialogue('powerup')
                                elif ptype == 'slow_motion':
                                    active_powerups['slow_motion'] = POWERUP_DURATION_STANDARD
                                    floating_texts.append(FloatingText(powerup.x, powerup.y, "¡CÁMARA LENTA!", (100, 150, 255)))
                                    dialogue_system.add_combat_dialogue('powerup')
                                elif ptype == 'extra_ammo':
                                    ammo = min(ammo + 5, 99)
                                    ...
                                    floating_texts.append(FloatingText(powerup.x, powerup.y, "¡IMÁN!", PURPLE))
                                    dialogue_system.add_combat_dialogue('powerup')
                        ...
                        if not hit and ammo == 0:
                            dog.show_laugh()
                            dialogue_system.add_combat_dialogue('low_ammo')
                    ...
                    if event.key == pygame.K_SPACE:
                        ...
                        if secret_mode:
                            ...
                            if round_num > 5:
                                ...
                                ducks_spawned = 1
                                ammo = 3
                                # Para modo secreto, cada ronda es un boss diferente
                                boss_index = min(round_num - 1, len(SECRET_BOSSES) - 1)
                                ducks = [Duck(difficulty=round_num + 1, is_boss=True, boss_data=SECRET_BOSSES[boss_index])]
                                game_state = 'playing'
                                dialogue_system.add_combat_dialogue('boss_spawn')
        ...
        if game_state == 'intro':
            ...
            # Actualizar sistema de diálogos
            ...
            if qte_result is not None:
                ...
                if qte_result > 0:
                    ...
                    floating_texts.append(FloatingText(SCREEN_WIDTH // 2, 250, f"{qte_result} PENALIZACIÓN!", RED))
                ...
                if ducks_spawned >= ducks_per_round:
                    ...
                    if secret_mode:
                        if score >= required_score:
                            ...
                            dialogue_system.add_combat_dialogue('perfect_round')
                        else:
                    ...
                    if secret_mode:
                        ...
                        is_boss = (ducks_spawned + 1) == ducks_per_round  # Último es boss
                        boss_index = min(round_num - 1, len(SECRET_BOSSES) - 1)
                        ducks.append(Duck(difficulty=difficulty, is_boss=is_boss, boss_data=SECRET_BOSSES[boss_index] if is_boss else None))
                    else:
        ...
        if game_state == 'intro':
            ...
            # Función simplificada para modo secreto
            screen.fill((0, 0, 0))
            if secret_intro_stage == 0:
                text = font_large.render("Descubriendo el camino secreto...", True, GOLD)
                screen.blit(text, (SCREEN_WIDTH // 2 - text.get_width() // 2, SCREEN_HEIGHT // 2))
            elif secret_intro_stage == 1:
                text = font_large.render("La cueva de los patos oscuros...", True, (200, 100, 50))
                screen.blit(text, (SCREEN_WIDTH // 2 - text.get_width() // 2, SCREEN_HEIGHT // 2))
            else:
                text = font_large.render("¡Bienvenido al Infierno!", True, (255, 50, 50))
                screen.blit(text, (SCREEN_WIDTH // 2 - text.get_width() // 2, SCREEN_HEIGHT // 2))
            
            if chapter_frame > 90:
                pulse = int(128 + 127 * math.sin(chapter_frame * 0.12))
                continue_text = font_small.render("ESPACIO para continuar", True, (pulse, pulse, pulse))
                screen.blit(continue_text, (SCREEN_WIDTH // 2 - continue_text.get_width() // 2, SCREEN_HEIGHT - 60))
        
        elif game_state == 'boss_presentation' or showing_boss_intro:
            # Función simplificada para presentación de boss
            screen.fill((0, 0, 0))
            if current_boss_index < len(SECRET_BOSSES):
                boss = SECRET_BOSSES[current_boss_index]
                text1 = font_title.render(boss["name"], True, boss["color"])
                screen.blit(text1, (SCREEN_WIDTH // 2 - text1.get_width() // 2, 150))
                
                text2 = font_medium.render(boss["description"], True, WHITE)
                screen.blit(text2, (SCREEN_WIDTH // 2 - text2.get_width() // 2, 230))
                
                text3 = font_small.render(f"NIVEL {current_boss_index + 1}", True, GOLD)
            ...
            # ANIMACIÓN DE DESBLOQUEO MODO SECRETO
            if showing_unlock_animation:
                ...
                # Manejo de input para la animación
                for event in pygame.event.get():
            ...
            if game_state == 'round_end':
                ...
                text3 = font_medium.render(f"Puntuación: {score:,}", True, GOLD)
                ...
                text2 = font_medium.render(f"Puntuación Final: {score:,}", True, WHITE)
                screen.blit(text2, (SCREEN_WIDTH // 2 - text2.get_width() // 2, panel_y + 100))
                
                if secret_mode:
                    req_text = font_small.render(f"Requerías: {required_score} puntos", True, (255, 150, 150))
                    screen.blit(req_text, (SCREEN_WIDTH // 2 - req_text.get_width() // 2, panel_y + 140))
                
                if score > high_score:
                    new_record = font_medium.render("¡NUEVO RÉCORD!", True, GOLD)
                    screen.blit(new_record, (SCREEN_WIDTH // 2 - new_record.get_width() // 2, panel_y + 175))
                else:
                    hi_text = font_small.render(f"Récord: {high_score:,}", True, (150, 150, 150))
                    screen.blit(hi_text, (SCREEN_WIDTH // 2 - hi_text.get_width() // 2, panel_y + 180))
                
                text3 = font_small.render("Presiona ESPACIO para volver al menú", True, (180, 180, 180))
            ...
            if secret_mode:
                ...
                text2 = font_large.render("¡Has completado todos los capítulos!", True, WHITE)
            screen.blit(text2, (SCREEN_WIDTH // 2 - text2.get_width() // 2, title_y + 90))
            
            text3 = font_large.render(f"Puntuación Final: {score:,}", True, GOLD)
            screen.blit(text3, (SCREEN_WIDTH // 2 - text3.get_width() // 2, title_y + 160))
            
            if score > high_score:
                record_text = font_medium.render("¡NUEVO RÉCORD MUNDIAL!", True, (255, 100, 100))
                screen.blit(record_text, (SCREEN_WIDTH // 2 - record_text.get_width() // 2, title_y + 220))
            
            text4 = font_small.render("Presiona ESPACIO para volver al menú", True, (180, 180, 180))
